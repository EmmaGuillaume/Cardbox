...
function SearchContent() {
  ...
  const fetchResults = useCallback(async (q: string, p: number) => {
    ...
    try {
      ...
      const all: TMDBResult[] = data.results ?? [];
  ...
  const filtered = results.filter((r) =>
    filter === "all" ? true : r.media_type === filter
  );

  const getYear = (r: TMDBResult) => {
    ...
    >
      ...
      <div className="mt-8">
        <h1 className="text-primary font-merryweather flex gap-2 text-2xl flex-wrap">
          Résultats pour{" "}
          <span className="text-yellow">« {query || "…"} »</span>
        </h1>
      ...
      {/* Gens */}
      {!isLoading && people.length > 0 && (filter === "all" || filter === "person") && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {filtered.map((result) => (
              <FilmCard
                ...
                realisateur={result.media_type === "tv" ? "Série" : "Film"}
                date={getYear(result)}
                imageURL={
                  result.poster_path
                    ? `${TMDB_IMAGE_BASE}${result.poster_path}`
                    : "https://placehold.co/300x450?text=N/A"
                }
                isLiked={false}
                isInWatchlist={false}
                isSeen={false}
      ...
      {!isLoading && filtered.length === 0 && people.length === 0 && query && (
        <div className="flex flex-col items-center justify-center py-24 gap-3 opacity-40">
          ...
          <p className="font-merryweather text-lg">Aucun résultat pour « {query} »</p>
          <p className="font-extralight text-sm">Essaie un autre titre ou nom d'artiste</p>
        </div>
      )}
